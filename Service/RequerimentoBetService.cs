...
{
    public class RequerimentoBetService : IRequerimentoBet
    {
        private readonly string _stringConexao;

        public RequerimentoBetService(IConfiguration configuracao)
        {
        ...
        public async Task CadastrarModeloRequerimento(ModeloRequerimentoDto dto)
        {
            try
            {
                ...
                var query = @"
                              ...
                              VALUES (@nome, @arquivo, 'RequerimentoBet');
                            ";
        ...
        public async Task<List<ModeloRequerimentoDownloadDto>> ObterTodosModelos()
        {
            try
            {
                ...
                var query = "SELECT id, nome FROM modelos_requerimento WHERE categoria = 'RequerimentoBet';";
                await using var cmd = new NpgsqlCommand(query, conexao);
        ...
        public async Task<BaixarModeloRequerimentoDto> BaixarModeloPorId(int id)
        {
            ...
            {   
                ...
                var query = "SELECT nome, arquivo FROM modelos_requerimento WHERE id = @id AND categoria = 'RequerimentoBet';";
                await using var cmd = new NpgsqlCommand(query, conexao);
        ...
        public async Task EnviarRequerimento(EnviarRequerimentoDto dto, int usuarioId)
        {
            try
            {
                ...
                var query = @"
                    ...
                    VALUES (@usuario_id, @protocolo, @tipo, @observacoes, @arquivo, @nome_arquivo, @data_hora, @status, 'RequerimentoBet');
                ";
        ...
        public async Task<List<RequerimentoResumoDto>> ObterTodosRequerimentos(int usuarioId)
        {
            try
            {
                ...
                FROM requerimentos
                WHERE usuario_id = @usuario_id And categoria = 'RequerimentoBet'
                ...
                while (await reader.ReadAsync())
                {
                    lista.Add(new RequerimentoResumoDto
                    {
                        ...
                        NomeArquivo = reader.IsDBNull(6) ? null : reader.GetString(6) // <-- novo campo
                    });
                }
        ...
        public async Task DeletarModeloPorId(int id)
        {
            try
            {
                ...
                var verificarQuery = "SELECT COUNT(*) FROM modelos_requerimento WHERE id = @id And categoria ='RequerimentoBet';";
